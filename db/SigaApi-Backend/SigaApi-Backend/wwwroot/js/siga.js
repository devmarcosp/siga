...
function buildNav() {
    const menus = {
        ADMIN: [ { id: 'dashboard', icon: '📊', label: 'Panel Principal' }, { id: 'cursos', icon: '📚', label: 'Cursos' }, { id: 'docentes', icon: '👨‍🏫', label: 'Docentes' }, { id: 'estudiantes', icon: '👥', label: 'Alumnos' }, { id: 'apoderados', icon: '👨‍👧', label: 'Apoderados' }, { id: 'alertas', icon: '⚠️', label: 'Alertas' } ],
...
async function render() {
    ...
    const renderers = {
        ADMIN_dashboard: renderAdminDashboard, ADMIN_cursos: renderAdminCursos, ADMIN_docentes: renderAdminDocentes, ADMIN_estudiantes: renderAdminEstudiantes, ADMIN_apoderados: renderAdminApoderados, ADMIN_alertas: renderAdminAlertas,
...
async function saveDocente(e) {
    ...
    if (AppCache.users.find(u => u.identificador === correo)) {
...
function updateAttStatus(idCurso, idEstudiante) {
    ...
    const registro = AppCache.asistencia.find(a => a.idEstudiante === idEstudiante && a.fecha === fecha);
    ...
    <form id="formStudent" onsubmit="saveStudent(event)" class="p-6 space-y-4">
        ...
        <div>
            ...
            <select id="modalEstCurso" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:border-indigo-500 outline-none">
                ...
                ${AppCache.cursos.map(c => `<option value="${c.idCurso}">${c.nombreCurso}</option>`).join('')}
            </select>
        </div>
        <div>
...
async function saveStudent(e) {
    ...
    const curso = AppCache.cursos.find(c => c.idCurso === idCurso);
    ...
    if (!apoderados.length) {
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-100">
                    ${apoderados.map(a => {
                        const pupilo = AppCache.estudiantes.find(e => e.idEstudiante === a.pupiloId);
                        return `
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-3">
                                <div class="flex items-center gap-3">
...
async function renderDocenteDashboard() {
    ...
    const recentAsistencia = AppCache.asistencia.slice(-5).reverse(); 
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                ...
                <div class="space-y-2">
                    ${recentAsistencia.length ? recentAsistencia.map(a => {
                        const est = AppCache.estudiantes.find(e => e.idEstudiante === a.idEstudiante);
...
async function openEditGradeModal(idCalificacion) {
    ...
    const grade = AppCache.calificaciones.find(c => c.idCalificacion === idCalificacion);
    if(!grade) return;
...
async function renderMaterialsList(idCurso) {
    ...
    container.innerHTML = `<div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-gray-100">
                ${list.map(m => {
                    const curso = AppCache.cursos.find(c => c.idCurso === m.idCurso);
...
function downloadMaterial(idMaterial, titulo) {
    const material = AppCache.materiales.find(m => m.idMaterial === idMaterial);
    ...
    // ✅ Si es un archivo de ejemplo (sin contenido real)
        ...
        const nombreCurso = AppCache.cursos.find(c => c.idCurso === material.idCurso)?.nombreCurso || 'General';
        const contenido = [
            `========================================`,
...
async function openEditStudentModal(idEstudiante) {
    const estudiante = AppCache.estudiantes.find(e => e.idEstudiante === idEstudiante);
...
async function saveEditedStudent(e, idEstudiante) {
    ...
    const cursoObj = AppCache.cursos.find(c => c.idCurso === idCurso);
...
async function openEditDocenteModal(identificador) {
    const docente = AppCache.users.find(u => u.rol === 'DOCENTE' && u.identificador === identificador);
    if (!docente) return;
    const m = document.getElementById('modal');
    document.getElementById('modalContent').innerHTML = `
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
...
async function saveEditedDocente(e, identificadorOriginal) {
    ...
    if (nuevoCorreo !== identificadorOriginal && AppCache.users.find(u => u.identificador === nuevoCorreo)) {
        toast('❌ Ya existe un usuario con ese correo');
        return;
    }
...
async function openEditApoderadoModal(identificador) {
    const apoderado = AppCache.users.find(u => u.rol === 'APODERADO' && u.identificador === identificador);
    if (!apoderado) return;
    const m = document.getElementById('modal');
    document.getElementById('modalContent').innerHTML = `
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
